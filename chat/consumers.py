...
class ChatConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        from .models import Room  # Lazy import here
    ...
    async def receive(self, text_data):
        ...
        if data.get('type') == 'load_more':
            ...
            await self.send(text_data=json.dumps({
                'type': 'history',
                'messages': [{'id': msg.id, 'message': msg.content, 'username': msg.username, 
                             'timestamp': msg.created_at.strftime('%H:%M')} for msg in messages]
            }))
            return
    ...
    def get_last_messages(self):
        from .models import Room, Message  # Lazy import here
